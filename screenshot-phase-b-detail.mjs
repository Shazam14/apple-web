...
const BORROWER_ID = process.env.BID ?? "36";
...
for (const s of sizes) {
  ...
  await page.goto(`http://localhost:3002/borrowers/${BORROWER_ID}`, { waitUntil: "networkidle" });
  await page.waitForTimeout(700);
  await page.screenshot({
    path: `/root/projects/apple-website/shot-phase-b-detail-${s.name}.png`,
    fullPage: false,
  });
  await page.screenshot({
    path: `/root/projects/apple-website/shot-phase-b-detail-${s.name}-full.png`,
    fullPage: true,
  });
  await ctx.close();
}
