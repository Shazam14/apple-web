...
for (const s of sizes) {
  ...
  await page.goto("http://localhost:3002/money", { waitUntil: "networkidle" });
  await page.waitForFunction(() => !document.body.innerText.includes("Loading…"), { timeout: 15000 });
  await page.waitForTimeout(500);
  await page.screenshot({
    path: `/root/projects/apple-website/shot-phase-b-money-${s.name}.png`,
    fullPage: false,
  });
  await page.screenshot({
    path: `/root/projects/apple-website/shot-phase-b-money-${s.name}-full.png`,
    fullPage: true,
  });
  await ctx.close();
}
